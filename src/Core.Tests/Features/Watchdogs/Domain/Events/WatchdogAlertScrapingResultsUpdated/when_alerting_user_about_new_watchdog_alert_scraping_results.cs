...
public class when_alerting_user_about_new_watchdog_alert_scraping_results : BaseDatabaseTest
{
    ...
    public void email_alert_with_new_scraping_results_is_sent_to_user()
    {
        A.CallTo(() => _emailSender.SendEmail(
                _user.Email,
                A<string>.That.Matches(p => p.Contains("New results for Mr Watchdog alert") && p.Contains("Epic Games store free game")),
                A<string>.That.Matches(p => p.Contains("new results for your alert")
                                            && p.Contains($"""
