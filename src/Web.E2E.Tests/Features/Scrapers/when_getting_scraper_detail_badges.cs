...
[TestFixture]
public class when_getting_watchdog_detail_badges : BaseDatabaseTest
{
    ...
    {
        var url = WatchdogUrlConstants.WatchdogDetailBadgesUrlTemplate.WithWatchdogId(_watchdog!.Id);
        var response = await RunOncePerTestRun.SharedWebApplicationClient.Value.GetAsync(url);
        response.StatusCode.ShouldBe(HttpStatusCode.Redirect);
        response.Headers.Location.ShouldNotBeNull();
        response.Headers.Location.ToString().ShouldEndWith($"/Account/Login?ReturnUrl=%2FWatchdogs%2FDetail%2FBadges%2F{_watchdog!.Id}");
    }
