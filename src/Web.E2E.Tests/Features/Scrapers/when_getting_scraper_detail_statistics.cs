...
[TestFixture]
public class when_getting_scraper_detail_statistics : BaseDatabaseTest
{
    ...
    [Test]
    public async Task scraper_detail_statistics_page_redirects_to_login_page()
    {
        var url = ScraperUrlConstants.ScraperDetailStatisticsUrlTemplate.WithScraperId(_scraper!.Id);
        var response = await RunOncePerTestRun.SharedWebApplicationClient.Value.GetAsync(url);
        response.StatusCode.ShouldBe(HttpStatusCode.Redirect);
        response.Headers.Location.ShouldNotBeNull();
        response.Headers.Location.ToString().ShouldEndWith($"/Account/Login?ReturnUrl=%2FScrapers%2FDetail%2FStatistics%2F{_scraper!.Id}");
    }
