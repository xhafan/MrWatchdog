...
public class when_getting_watchdog_search_page : BaseDatabaseTest
{
    ...
    {
        var url = ScraperUrlConstants.WatchdogSearchUrlTemplate.WithWatchdogSearchId(_watchdogSearch!.Id);
        var response = await RunOncePerTestRun.SharedWebApplicationClient.Value.GetAsync(url);
        response.StatusCode.ShouldBe(HttpStatusCode.Redirect);
        response.Headers.Location.ShouldNotBeNull();
        response.Headers.Location.ToString().ShouldEndWith($"/Account/Login?ReturnUrl=%2FScrapers%2FSearch%2F{_watchdogSearch.Id}");
    }
