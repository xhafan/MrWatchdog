...
[TestFixture]
public class when_getting_watchdog_detail_statistics : BaseDatabaseTest
{
    ...
    [Test]
    public async Task watchdog_detail_statistics_page_redirects_to_login_page()
    {
        var url = WatchdogUrlConstants.WatchdogDetailStatisticsUrlTemplate.WithWatchdogId(_watchdog!.Id);
        var response = await RunOncePerTestRun.SharedWebApplicationClient.Value.GetAsync(url);
        response.StatusCode.ShouldBe(HttpStatusCode.Redirect);
        response.Headers.Location.ShouldNotBeNull();
        response.Headers.Location.ToString().ShouldEndWith($"/Account/Login?ReturnUrl=%2FWatchdogs%2FDetail%2FStatistics%2F{_watchdog!.Id}");
    }
