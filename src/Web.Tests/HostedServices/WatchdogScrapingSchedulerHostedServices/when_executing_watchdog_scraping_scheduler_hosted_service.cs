...
public class when_executing_watchdog_scraping_scheduler_hosted_service : BaseDatabaseTest
{
    ...
    private Watchdog _watchdogWithNextScrapingOnSetInThePast = null!;
    ...
    public async Task Context()
    {
        ...
        A.CallTo(() => watchdogScrapingSchedulerHostedServiceOptions.Value).Returns(new WatchdogScrapingSchedulerHostedServiceOptions {IsDisabled = false});
    ...
    [Test]
    public void watchdog_with_next_scraping_on_timestamp_in_the_past_has_scraping_scheduled()
    {
        A.CallTo(() => _bus.Send(new ScrapeWatchdogCommand(_watchdogWithNextScrapingOnSetInThePast.Id))).MustHaveHappenedOnceExactly();
    } 
    ...
    public async Task TearDown()
    {
        ...
        watchdog = await watchdogRepository.GetAsync(_watchdogWithNextScrapingOnSetInThePast.Id);
        if (watchdog != null)
        {
            await watchdogRepository.DeleteAsync(watchdog);
        }
    ...
    private void _BuildEntities()
    {
        ...
        _watchdogWithNextScrapingOnSetInThePast = new WatchdogBuilder(UnitOfWork)
            .WithNextScrapingOn(DateTime.UtcNow.AddSeconds(-1))
            .Build();
